...
const currency = transpile(
  readFileSync(new URL('../src/utils/currency.ts', import.meta.url), 'utf8')
);
const source = readFileSync(
  new URL('../src/utils/split.ts', import.meta.url),
  'utf8'
).replace("'./currency'", JSON.stringify(currency));
const {
  computeNetBalances,
  resolveOwedPerUser,
  allocateMoney,
...
for (const splitType of ['equal', 'exact', 'percentage', 'shares']) {
  for (const rate of [1, 1.1, 0.3333, 1.2345]) {
    test(`${splitType}, rate ${rate}, multiple payers conserve total`, () => {
      ...
      const expense = {
        originalAmount: 10,
        convertedAmount: Math.round(10 * rate * 100) / 100,
        originalCurrency: 'EUR',
        groupCurrency: rate === 1 ? 'EUR' : 'USD',
        rateSnapshot: { rate },
        splitType,
        participants: values.map((value, i) => ({ userId: String(i), value })),
        paidBy: [
          { userId: '0', amount: 3.33 },
          { userId: 'payer', amount: 6.67 },
        ],
      };
      assert.deepEqual(validateSplit(expense), []);
      const owed = Object.values(
        resolveOwedPerUser(
  ...
  for (let total = 1; total < 200; total++) {
    for (let count = 1; count < 40; count++) {
      const shares = allocateMoney(
        total / 100,
        Array.from({ length: count }, (_, i) => i % 7)
      );
      if (count === 1) continue;
...
test('rejects invalid money and duplicate identities', () => {
  ...
  assert.ok(
    validateSplit({
      originalAmount: 1,
      paidBy: [{ userId: 'a', amount: 1 }],
      participants: [{ userId: 'a', value: -1 }],
      splitType: 'exact',
    }).length
  );
});
