  ...
  Object.keys(api)
    .map(
      (key) =>
        'export const ' +
        key +
        ' = (...args) => globalThis.__friendsTest.api.' +
        key +
        '(...args)'
    )
    .join('\n')
...
const config = url(
  'export const db = {}; export const auth = globalThis.__friendsTest.auth'
);
const userApi = url(
  'export const getUserByEmail = async email => globalThis.__friendsTest.users[email]'
);
let source = readFileSync(
  new URL('../src/api/friends.ts', import.meta.url),
  'utf8'
);
...
const { sendFriendRequest, respondToFriendship, friendshipId } = await import(
  url(compiled)
);
function reset(uid = 'a') {
  docs = new Map();
...
test('only the recipient can accept, and only once', async () => {
  ...
  await assert.rejects(
    respondToFriendship('a:b', 'accept'),
    /no longer available/
  );
  ...
  await assert.rejects(
    respondToFriendship('a:b', 'remove'),
    /Permission denied/
  );
