...
const { calculateBalances, aggregateNetBalances, simplifyDebts } = await import(
  balances
);
const { summarizeDashboard } = await import(
  compile(
    read('../src/utils/dashboard.ts').replace(
      "'./balances'",
...
test('paid, owed, net and suggestions agree for a shared dinner', () => {
  ...
  assert.deepEqual(
    rows.find((r) => r.userId === 'a'),
    { userId: 'a', paid: 90, owed: 30, settlementNet: 0, net: 60 }
  );
  assert.deepEqual(
    simplifyDebts(aggregateNetBalances([expense()], [], 'EUR')),
    [
...
test('partial and full settlements reduce debts without inflating expense totals', () => {
  ...
  assert.deepEqual(
    [...aggregateNetBalances([expense()], [transfer], 'EUR')],
    [
      ['a', 50],
      ['b', -20],
      ['c', -30],
  ...
  const settlements = simplifyDebts(
    aggregateNetBalances([expense()], [], 'EUR')
  ).map((s) => ({
    fromUserId: s.from,
    toUserId: s.to,
    amount: s.amount,
  ...
  assert.ok(
    calculateBalances([expense()], settlements, 'EUR').every((r) => r.net === 0)
  );
  assert.equal(
    calculateBalances([expense()], settlements, 'EUR').reduce(
      (sum, r) => sum + r.paid,
      0
    ),
    90
...
test('FX uses saved converted cents and many tiny expenses remain zero-sum', () => {
  ...
  assert.equal(
    [...net.values()].reduce((sum, n) => sum + Math.round(n * 100), 0),
    0
  );
...
test('mixed currencies, malformed splits and unbalanced nets fail closed', () => {
  assert.throws(
    () => calculateBalances([expense({ groupCurrency: 'USD' })], [], 'EUR'),
    /invalid/
  );
  assert.throws(
    () => calculateBalances([expense({ paidBy: [] })], [], 'EUR'),
    /invalid/
  );
  assert.throws(
    () =>
      calculateBalances(
        [],
        [{ currency: 'EUR', fromUserId: 'a', toUserId: 'a', amount: 1 }],
        'EUR'
      ),
    /invalid/
  );
...
test('dashboard sums across groups while keeping currencies and debts separate', () => {
  const groups = [
    ...
    {
      currency: 'EUR',
      expenses: [expense({ paidBy: [{ userId: 'b', amount: 90 }] })],
      settlements: [],
    },
    {
      currency: 'USD',
      expenses: [expense({ groupCurrency: 'USD' })],
      settlements: [],
    },
  ];
