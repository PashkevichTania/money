...
const api = {
  ...
  runTransaction: async (_, action) => {
    ...
    const result = await action({
      ...
      set: (reference, value) =>
        operations.push(() => docs.set(reference.path, structuredClone(value))),
...
const firestore = compile(
  Object.keys(api)
    .map(
      (key) =>
        `export const ${key} = (...args) => globalThis.__settlementApi.${key}(...args)`
    )
    .join('\n')
);
let source = readFileSync(
  new URL('../src/api/settlements.ts', import.meta.url),
  'utf8'
);
for (const [key, value] of [
  ...
  [
    '@/config/firebase',
    compile(
      'export const db = {}; export const isFirebaseConfigured = true; export const auth = globalThis.__settlementAuth'
    ),
  ],
  [
    '@/utils/dates',
    compile("export const nowIso = () => '2026-09-20T12:00:00.000Z'"),
  ],
])
...
test('records partial payment in group currency with one write when history exists', async () => {
  ...
  assert.equal(saved.createdBy, 'b');
  assert.equal(saved.note, 'Thanks');
...
test('sender or recipient can record, but third member and anonymous caller cannot', async () => {
  ...
  for (const user of [{ uid: 'c' }, null]) {
    ...
    await assert.rejects(
      createSettlement('forbidden', input()),
      /sender or recipient|signed in/
    );
  }
  assert.equal(writes, 1);
});
...
test('uncertain save can be retried with the same identifier without duplicating payment', async () => {
  ...
  await assert.rejects(
    createSettlement('stable-id', input()),
    /Connection lost/
  );
  ...
  assert.equal(writes, 1);
  await assert.rejects(
    createSettlement('stable-id', input({ amount: 20 })),
    /different details/
  );
