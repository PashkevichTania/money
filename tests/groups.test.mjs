...
const snapshot = (reference) => ({
  ...
  get: (key) => docs.get(reference.path)?.[key],
});
const api = {
  runTransaction: async (_, work) =>
    work({
      get: async (reference) => snapshot(reference),
      update: async (reference, patch) => api.updateDoc(reference, patch),
    }),
  collection: (_, ...parts) => ref(parts.join('/')),
  doc: (parent, ...parts) =>
    ref(
      parent.path
        ? `${parent.path}/${parts.join('/') || 'new-group'}`
        : parts.join('/')
    ),
  ...
  getDocs: async (reference) => {
    const count =
      reference.constraints?.find((c) => c.count)?.count ?? Infinity;
    const children = [...docs.keys()]
      .filter(
        (p) =>
          p.startsWith(reference.path + '/') &&
          p.split('/').length === reference.path.split('/').length + 1
      )
      .slice(0, count)
  ...
  writeBatch: () => {
    const children = [];
    return {
      delete: (reference) => children.push(reference),
      commit: async () => {
        if (failBatch) {
          failBatch = false;
        ...
        for (const child of children) {
          docs.delete(child.path);
          events.push(['delete', child.path]);
        }
      },
    };
  },
...
const firestore = url(
  Object.keys(api)
    .map(
      (key) =>
        `export const ${key} = (...args) => globalThis.__groupTestFirestore.${key}(...args)`
    )
    .join('\n')
);
globalThis.__groupAuth = { currentUser: { uid: 'a' } };
const config = url(
  'export const db = {}; export const isFirebaseConfigured = true; export const auth = globalThis.__groupAuth'
);
const dates = url("export const nowIso = () => '2026-09-19T00:00:00.000Z'");
let source = readFileSync(
  new URL('../src/api/groups.ts', import.meta.url),
  'utf8'
);
...
test('currency cannot change even for an empty group', async () => {
  ...
  await assert.rejects(
    updateGroup('g', { baseCurrency: 'USD' }),
    /cannot be changed/
  );
