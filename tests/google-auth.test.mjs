...
const user = { uid: 'google-user', displayName: 'Google User', email: 'USER@EXAMPLE.COM', photoURL: 'https://example.com/avatar.png' }
...
const authAdapter = {
  ...
  signOut: async () => {}, onAuthStateChanged: () => () => {}, updateProfile: async () => {},
}
globalThis.__googleAuthAdapter = authAdapter
globalThis.__googleProfileAdapter = {
  doc: (_db, collection, id) => ({ collection, id }),
  getDoc: async () => ({ exists: () => !!stored, data: () => stored }),
  setDoc: async (_ref, data, options) => { stored = options?.merge ? { ...stored, ...data } : data },
}
const authModule = url(Object.keys(authAdapter).map(key => `export const ${key} = (...args) => globalThis.__googleAuthAdapter.${key}(...args)`).join('\n') + '\nexport class GoogleAuthProvider { setCustomParameters(parameters) { this.parameters = parameters } }')
const firestoreModule = url(['doc', 'getDoc', 'setDoc'].map(key => `export const ${key} = (...args) => globalThis.__googleProfileAdapter.${key}(...args)`).join('\n'))
async function loadStore(configured) {
  let source = readFileSync(new URL('../src/stores/authStore.ts', import.meta.url), 'utf8')
  for (const [name, value] of [
    ['zustand', import.meta.resolve('zustand')], ['firebase/auth', authModule], ['firebase/firestore', firestoreModule],
    ['@/config/firebase', url(`export const auth = {}; export const db = {}; export const isFirebaseConfigured = ${configured}`)],
    ['@/config/currencies', url("export const DEFAULT_BASE_CURRENCY = 'USD'")],
    ['@/utils/dates', url("export const nowIso = () => '2026-09-20T00:00:00.000Z'")],
...
function reset() {
  stored = null; popupError = null; providerParameters = null; popupCalls = 0
  store.setState({ status: 'unauthenticated', profile: null, firebaseUser: null, error: null })
}
test('Google sign-in creates the standard profile and selects an account', async () => {
  ...
  assert.deepEqual(stored, { id: user.uid, displayName: user.displayName, email: 'user@example.com', photoURL: user.photoURL, defaultCurrency: 'USD', createdAt: '2026-09-20T00:00:00.000Z' })
...
test('returning Google user keeps their profile and currency preference', async () => {
  reset()
  stored = { id: user.uid, displayName: 'My custom name', email: 'user@example.com', defaultCurrency: 'EUR', createdAt: '2024-01-01' }
...
test('blocked popup, disabled provider, network and domain failures are actionable', async () => {
  for (const [code, message] of [
    ['auth/popup-blocked', /Allow pop-ups/], ['auth/operation-not-allowed', /not enabled/],
    ['auth/unauthorized-domain', /not available on this site/], ['auth/network-request-failed', /internet connection/],
...
test('an in-progress sign-in cannot open a second popup', async () => {
  reset(); store.setState({ status: 'loading' })
  await store.getState().loginWithGoogle()
...
test('unconfigured workspace reports an error without calling Google', async () => {
  ...
  await assert.rejects(unconfigured.getState().loginWithGoogle(), /workspace is configured/)
