...
const api = {
  doc: (parent, ...parts) =>
    ref(
      parent.path
        ? `${parent.path}/${parts.join('/') || 'new-expense'}`
        : parts.join('/')
    ),
  ...
  getDocs: async (reference) => ({
    forEach: (callback) =>
      [...docs]
        ...
        .forEach(([, value]) => callback({ data: () => value })),
  }),
  runTransaction: async (_, fn) => {
    ...
    await fn({
      ...
      set: (reference, value) =>
        writes.push(() => docs.set(reference.path, structuredClone(value))),
    });
...
const firestore = compile(
  Object.keys(api)
    .map(
      (key) =>
        `export const ${key} = (...args) => globalThis.__expenseApiTest.${key}(...args)`
    )
    .join('\n')
);
...
for (const [key, value] of [
  ...
  [
    '@/config/firebase',
    compile(
      'export const db = {}; export const isFirebaseConfigured = true; export const auth = globalThis.__expenseAuth'
    ),
  ],
  ['@/types/expense', compile(read('../src/types/expense.ts'))],
  ['@/utils/currency', currency],
  ['@/utils/split', split],
  [
    '@/utils/dates',
    compile("export const nowIso = () => '2026-09-20T00:00:00.000Z'"),
  ],
])
  source = source.replaceAll(`'${key}'`, JSON.stringify(value));
const { createExpense, updateExpense, listExpenses, deleteExpense } =
...
const reset = () => {
  docs = new Map([
    [
      'groups/g',
      { baseCurrency: 'EUR', memberIds: ['a', 'b'], hasExpenseHistory: false },
    ],
  ]);
...
test('expense create/list/edit/delete round trip retains group expense history', async () => {
  ...
  const updated = await updateExpense('g', expense.id, {
    title: 'Lunch',
    updatedBy: 'a',
  });
...
test('failed validation never marks history or writes an expense', async () => {
  for (const patch of [
    ...
    { splitType: 'unknown' },
    { convertedAmount: 99 },
    { paidBy: [{ userId: 'a', amount: 80 }] },
    { participants: [{ userId: 'stranger', value: 100 }] },
    { groupCurrency: 'USD', originalCurrency: 'USD' },
  ]) {
    reset();
...
test('FX creation and metadata edit preserve the saved exchange rate', async () => {
  ...
  const expense = await createExpense({
    ...
    convertedAmount: 90,
    rateSnapshot: { rate: 0.9, date: '2026-09-18', source: 'test' },
  });
  const next = await updateExpense('g', expense.id, {
    title: 'Updated',
    updatedBy: 'a',
  });
...
test('concurrent edits and deletion locks reject stale expense writes', async () => {
  ...
  const expense = await createExpense(input());
  concurrentChange = () => {
    docs.get(`groups/g/expenses/${expense.id}`).updatedAt = 'newer-version';
  };
  await assert.rejects(
    updateExpense('g', expense.id, { title: 'Overwrite', updatedBy: 'a' }),
...
test('non-authors and signed-out users cannot edit or delete expenses, even with spoofed updatedBy', async () => {
  ...
  try {
    for (const user of [{ uid: 'b' }, null]) {
      ...
      await assert.rejects(
        updateExpense('g', expense.id, { title: 'Forbidden', updatedBy: 'a' }),
        /Only the expense author/
      );
      await assert.rejects(
        deleteExpense('g', expense.id),
        /Only the expense author/
      );
...
test('expense type is optional, retained on unrelated edits, changeable and removable', async () => {
  ...
  assert.equal(
    (await updateExpense('g', expense.id, { title: 'Lunch', updatedBy: 'a' }))
      .type,
    'food'
  );
  assert.equal(
    (await updateExpense('g', expense.id, { type: 'housing', updatedBy: 'a' }))
      .type,
    'housing'
  );
  assert.equal(
    (await updateExpense('g', expense.id, { type: undefined, updatedBy: 'a' }))
      .type,
    undefined
  );
  await assert.rejects(
    createExpense({ ...input(), type: 'invalid' }),
