...
export function useExpenseParticipants({
  ...
  useEffect(() => {
    ...
    const validPayers = currentPaidBy.filter(({ userId }) =>
      memberIds.has(userId)
    );
    if (
      validPayers.length !== currentPaidBy.length ||
          ...
          : [
              {
                ...
                amount: Number(getValues('originalAmount')),
              },
            ],
        { shouldValidate: true }
      );
    ...
    const validParticipants = currentParticipants.filter((id) =>
      memberIds.has(id)
    );
  ...
  const allSelected =
    participantIds.length === members.length && members.length > 0;
  ...
  const toggleParticipant = (userId: string) => {
    ...
    if (
      ...
      const difference = roundMoney(
        100 - safeSum(Object.values(nextValues)),
        4
      );
      const lastId = nextIds.at(-1);
      if (lastId)
        nextValues[lastId] = roundMoney(nextValues[lastId] + difference, 4);
      setValue(
        'participantValues',
  ...
  const toggleAll = () => {
    setValue('participantIds', allSelected ? [] : members.map(({ id }) => id), {
      shouldValidate: true,
    });
  };

  const applySinglePayer = (userId: string) => {
    setValue('paidBy', [{ userId, amount: originalAmount }], {
      shouldValidate: true,
    });
  };
  ...
  const addPayer = (userId: string) => {
    if (!payerIds.has(userId)) {
      setValue('paidBy', [...paidBy, { userId, amount: 0 }], {
        shouldValidate: true,
      });
    }
  };
  ...
  const removePayer = (userId: string) => {
    if (paidBy.length > 1) {
      setValue(
        'paidBy',
        paidBy.filter((payer) => payer.userId !== userId),
        {
          shouldValidate: true,
        }
      );
